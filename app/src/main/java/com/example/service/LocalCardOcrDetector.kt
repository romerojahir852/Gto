...
object LocalCardOcrDetector {
    ...
    suspend fun detect(
        ...
        return try {
            ...
            // Paso 2A: SIEMPRE escanear P�ldora Digital Superior (PokerStars / BCPoker)
            try {
            ...
            // Paso 4: Si a�n hay < 5 cartas de mesa, re-escanear con CONTRASTE INVERTIDO (escala de grises + 1.5x)
            ...
            if (!heroChanged && bestBoard.size < currentState.cartasComunitarias.size) {
                // Fase NUNCA retrocede si Hero no cambi� � conservar mesa anterior (Punto 7)
            ...
            // Validaci�n Texas Hold'em: solo 0, 3, 4, 5 cartas de mesa son v�lidas (Punto 25)
            ...
            // Validaci�n de unicidad del mazo: no puede haber cartas duplicadas (Punto 23)
    ...
    /**
     * Normaliza tokens OCR corrigiendo confusiones comunes de MLKit en fuentes de p�ker m�vil.
